...
def parse_blocks(text):
    """The ```nlang fences of a markdown page, in document order."""
    blocks = []
    ...
    while i < len(lines):
        ...
        if end is None:
            break               #unclosed fence: markdown would do the same
        if lang == "nlang":
...
def group_programs(blocks):
    """(programs, skipped) — labeled mainless fences join the preceding
    program; bare mainless fences are reported as skipped."""
    programs = []
    skipped = []
    for block in blocks:
        ...
        if has_main:
            ...
            if block.label:
                relpath = Path(block.label).as_posix()
            else:
                relpath = "snippet_%02d.n" % (len(programs) + 1)
            program = Program(Path(relpath).stem, block.line)
            program.add(relpath, block.code, True)
            programs.append(program)
        elif programs and block.label:
            programs[-1].add(block.label, block.code, False)
        else:
...
def audit_doc(doc_path, ncc, nvm, workdir):
    ...
    Returns a list of human-readable problems (empty = clean). Every
    program gets its own scratch directory, removed afterwards; the
    .nmod files stay inside it, so the source tree never gains build
    artifacts.
    """
    blocks = parse_blocks(Path(doc_path).read_text(encoding="utf-8"))
    if not blocks:
        raise ValueError(
            "%s: no ```nlang blocks found; an audit that checks nothing "
            "must not pass" % doc_path)
    programs, skipped = group_programs(blocks)
    problems = []
