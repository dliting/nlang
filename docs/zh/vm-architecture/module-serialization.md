...
ncc 产出的 v1.10 模块会把这些元素装箱进 GC 永不追踪的基元槽位，因
此加载器直接拒绝 minor < 11——旧模块必须重新编译。
v1.10（数组重设计 B）——一次语义下限抬升，不是布局变更：没有新增序
...
v1.9（调试器）——每个函数记录以一个 `sourceFile` 字符串收尾（其编
译自的翻译单元路径，位于 locals 块之后）；导入合并同时拷贝
`func.locals`，修复了被导入帧根集为空的 GC 根集空洞。
v1.8（Phase 13）——一等函数值：`RTK_Func` kind 字节加上前述 8 条函
数值指令；旧 VM 无法执行这些指令，会直接拒绝 v1.8 模块（下限在此步
升到 minor 8）。
v1.7（Phase 11）——stdlib 命名空间内建函数 + 保留命名空间；字段布
