...
溢出 C++ 调用栈。迭代方案用一个 `vector<int32_t>` 工作表：MarkPhase
识别根引用并压入工作表，然后迭代处理条目直至工作表清空；每个条目的
子引用若未标记则压入工作表。内存用量以此为界（O(可达对象数)），没有
栈溢出风险。
...
- **防御性 RTK_Array 元素分支**：数组分支会追踪声明 `elemKind` 为
  ...
  数组元素由此进入工作表的 `RTK_Array` 分支，按 elemKind 追踪数组自
