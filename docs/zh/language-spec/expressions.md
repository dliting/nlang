...
而 UTF-8 字节序等于码点序，因此对非 ASCII 文本排序同样正确：
`"é" > "z"` 为真。（Phase 11 Step 3b 之前，生成的代码比较的是 string
池索引——源码中字面量的出现顺序可能翻转结果。）
...
- string 与非 string 混用是**编译错误**（`"a" < 5`、`5 == "a"`）。
  ...
  读作池的 0 号条目（已知漏洞：内容恰好在池 0 号位的字符串与 null
  按位相同；池没有保留哨兵槽位）。
- int/float 对享受与算术相同的对称提升（Phase 8e-8）：`-2 < -1.5`
...
- `float → string`：`OP_Float_to_str`（`%g` 格式——`2.5` 而非
  `2.500000`）
- 两者都把格式化后的字符串压入运行期 `m_stringPool`，再把新索引写
...
编译器内嵌每枚举一张名字表；VM 用 `OP_Enum_to_str` 按值查成员名。
越界的枚举值在运行期抛错。
